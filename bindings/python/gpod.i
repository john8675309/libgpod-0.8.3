...
PyObject* sw_get_rule(GList *list, gint index) {
  ...
  position = g_list_nth(list, index);
  return SWIG_NewPointerObj((void*)(position->data), SWIGTYPE_p__Itdb_Device, 0);
 }
...
PyObject* sw_get_list_len(GList *list) {
   return PyLong_FromLong(g_list_length(list));
 }
...
PyObject* sw_get_photoalbum(GList *list, gint index) {
 ...
 PyObject* sw_get_photoalbum_members(Itdb_PhotoAlbum *album) {
  ...
  for (l = album->members, i = 0; l; l = l->next, ++i) {
    ...
    PyList_SET_ITEM(list, i, PyLong_FromLong((long)photo_id));
  }
  return list;
 }
...
#else
  if (PyDateTime_Check($input) || PyDate_Check($input)) {
#endif
    pydatetime = $input;
    Py_INCREF(pydatetime);
  } else if  ((PyLong_Check($input)) || (PyFloat_Check($input))) {
    PyObject* pyargs;
    Py_INCREF($input);
...
%typemap(in) guint8 {
   unsigned long ival;
   ival = PyLong_AsUnsignedLongMask($input);
   if (PyErr_Occurred())
        SWIG_fail;
   if (ival > 255) {
